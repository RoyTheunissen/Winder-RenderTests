...
namespace DeepSky.Haze
{
    ...
    public class DS_HazeView : MonoBehaviour
    {
        ...
        private Light cachedDirectLight;
        ...
        public Light DirectLight
        {
            get
            {
                if (cachedDirectLight == null)
                {
                    Light[] lights = FindObjectsOfType<Light>();
                    cachedDirectLight = lights.FirstOrDefault(l => l.type == LightType.Directional);
                }
                return cachedDirectLight;
            }
        }
        ...
        private void SetMaterialFromContext(DS_HazeContextItem ctx)
        {
            ...
            switch (DS_HazeCore.Instance.HeightFalloff)
            {
                ...
                case DS_HazeCore.HeightFalloffType.Exponential:
                    ...
                    fogViewpointDensity = Mathf.Exp(-ctx.m_FogDensityHeightFalloff * (absY - ctx.m_FogStartHeight));
                    break;
                default:
                    break;
            }
            ...
            Vector3 direction;
            Color lightColour;
            Shader.SetGlobalTexture("_DS_FogAmbientLightGradient", ctx.m_FogAmbientGradient);
            if (DirectLight)
            {
                direction = -DirectLight.transform.forward;
                lightColour = DirectLight.color.linear * DirectLight.intensity;

                Shader.SetGlobalColor("_DS_FogAmbientLight", ctx.m_FogAmbientColour.linear * DirectLight.intensity);
                Shader.SetGlobalColor("_DS_FogDirectLight", ctx.m_FogLightColour.linear * DirectLight.intensity);
            }
            else
            {
        ...
        {
            CommandBuffer[] cmds = DirectLight.GetCommandBuffers(LightEvent.AfterShadowMap);
            foreach (CommandBuffer cmd in cmds)
            {
        ...
        {
            CommandBuffer[] cmds = DirectLight.GetCommandBuffers(LightEvent.AfterScreenspaceMask);
            foreach (CommandBuffer cmd in cmds)
            {
        ...
        void UpdateResources()
        {
            ...
            if (DirectLight)
            {
                ...
                if (m_ShadowCascadesCmdBuffer == null)
                {
                    ...
                    DirectLight.AddCommandBuffer(LightEvent.AfterShadowMap, m_ShadowCascadesCmdBuffer);
                }
                ...
                if (m_DirectionalLightCmdBuffer == null)
                {
                        ...
                        DirectLight.AddCommandBuffer(LightEvent.AfterScreenspaceMask, m_DirectionalLightCmdBuffer);
                }
        ...
        void OnDisable()
        {
            ...
            if (DirectLight)
            {
                cmdBuffers = DirectLight.GetCommandBuffers(LightEvent.AfterShadowMap);
                foreach(CommandBuffer cmd in cmdBuffers)
                {
                    if (cmd.name == kShadowCascadesCmdBufferName)
                    {
                        DirectLight.RemoveCommandBuffer(LightEvent.AfterShadowMap, cmd);
                        break;
                    }
                }

                cmdBuffers = DirectLight.GetCommandBuffers(LightEvent.AfterScreenspaceMask);
                foreach (CommandBuffer cmd in cmdBuffers)
                {
                    if (cmd.name == kDirectionalLightCmdBufferName)
                    {
                        DirectLight.RemoveCommandBuffer(LightEvent.AfterScreenspaceMask, cmd);
                        break;
                    }
                }
            }
        ...
        void OnDestroy()
        {
            ...
            if (m_ShadowCascadesCmdBuffer != null)
            {
                if (DirectLight != null)
                {
                    DirectLight.RemoveCommandBuffer(LightEvent.AfterShadowMap, m_ShadowCascadesCmdBuffer);
                }
            ...
            if (m_DirectionalLightCmdBuffer != null)
            {
                if (DirectLight != null)
                {
                    DirectLight.RemoveCommandBuffer(LightEvent.AfterScreenspaceMask, m_DirectionalLightCmdBuffer);
                }
        ...
        void OnPreRender()
        {
            ...
            else
            {
                ...
                if (DirectLight && m_RenderAtmosphereVolumetrics)
                {
