...
namespace DeepSky.Haze
{
    ...
    {
        private static string[] MultiplierStr = {"0.1", "0.2", "0.5", "1", "2", "5", "10", "0.01" };
        ...
        private const string strToolTipFogStartHeight =
            "The height (global Y) at which the fog starts.";
        ...
        private const string strToolTipFogAmbGrad =
            "The ambient (or skylighting) colour of the fog when not directly illuminated by a light source. " +
            "Acts as a gradient for more stylized control of the colors. The left side is near, the right side is far.";
        ...
        private const float kControlLineCount = 23f;
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            Rect fHeight = new Rect(position.x, curY + ctrlSizeY * ctrlCount++, position.width, EditorGUIUtility.singleLineHeight);
            ...
            Rect fAmbGrad = new Rect(position.x, curY + ctrlSizeY * ctrlCount++, position.width, EditorGUIUtility.singleLineHeight);
            ...
            wasEnabled = GUI.enabled;
            if (falloff != DS_HazeCore.HeightFalloffType.Exponential) GUI.enabled = false;
            EditorGUI.PropertyField(fFalloff, property.FindPropertyRelative("m_FogDensityHeightFalloff"));
            GUI.enabled = wasEnabled;
            ...
            EditorGUI.PropertyField(fHeight, property.FindPropertyRelative("m_FogStartHeight"), new GUIContent("Start Height", strToolTipFogStartHeight));
            ...
            EditorGUI.PropertyField(fAmbGrad, property.FindPropertyRelative("m_FogAmbientGradient"), new GUIContent("Ambient Gradient", strToolTipFogAmbGrad));
